...
class LoraModel_PEFT(BaseAlgorithm):
    ...
    Args:
        ...
        self.adapter_name (`str`): The name of the adapter, defaults to `"default"`.
    ...
    **Attributes**:
    ...
    def _prepare_model(self, peft_config: LoraConfig_PEFT, model: nn.Module):
        r"""
    ...
    def _create_and_replace(
        ...
        if isinstance(target, LoraLayer) and not isinstance(target, AdaLoraLayer):
            ...
            new_module = self._create_new_module(
                lora_config, self.adapter_name, target, **kwargs
            )
            if self.adapter_name not in self.active_adapters:
        ...
        for name, module in new_module.named_modules():
            if (self.prefix in name) or ("ranknum" in name):
                weight = (
                    ...
                    else child.W_q if hasattr(child, "W_q") else child.weight
                )
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        Args:
            self.adapter_name (`str` or `list[str]`): Name of the adapter(s) to be activated.
        """
        for module in self.base_model.modules():
            if isinstance(module, LoraLayer):
                if module.merged:
                    ...
                    module.unmerge()
                module.set_adapter(self.adapter_name)
        self.active_adapter = self.adapter_name
    ...
    def _unload_and_optionally_merge(
        ...
        key_list = [
            key for key, _ in self.base_model.named_modules() if self.prefix not in key
        ]
    ...
    def add_weighted_adapter(
        ...
        if self.adapter_name in list(self.peft_config.keys()):
            return
        for adapter in adapters:
            if adapter not in list(self.peft_config.keys()):
                raise ValueError(f"Adapter {adapter} does not exist")
        ...
        self.peft_config[self.adapter_name] = replace(
            self.peft_config[adapters[0]],
            r=new_rank,
            lora_alpha=new_rank,
            target_modules=new_target_modules,
        )
        self.inject_adapter(self.base_model, self.adapter_name)

        # Do we really need that?
        _freeze_adapter(self.base_model, self.adapter_name)
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer):
                if self.adapter_name in target.lora_A:
                    target_lora_A = target.lora_A[self.adapter_name].weight
                    target_lora_B = target.lora_B[self.adapter_name].weight
                elif self.adapter_name in target.lora_embedding_A:
                    target_lora_A = target.lora_embedding_A[self.adapter_name]
                    target_lora_B = target.lora_embedding_B[self.adapter_name]
                else:
                    continue
    ...
    def delete_adapter(self, adapter_name: str) -> None:
        ...
        Args:
            self.adapter_name (str): Name of the adapter to be deleted.
        """
        if self.adapter_name not in list(self.peft_config.keys()):
            raise ValueError(f"Adapter {self.adapter_name} does not exist")
        del self.peft_config[self.adapter_name]
        ...
        for key in key_list:
            _, target, _ = _get_submodules(self.base_model, key)
            if isinstance(target, LoraLayer):
                target.delete_adapter(self.adapter_name)
                if new_adapter is None:
                    new_adapter = target.active_adapters[:]

        self.active_adapter = new_adapter or []

    def unload_and_merge(
        self,
        progressbar: bool = False,
        safe_merge: bool = False,
        adapter_names: Optional[list[str]] = None,
