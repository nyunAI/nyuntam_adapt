...
class Dataset:
    ...
    format_keys: Optional[str] = field(
        default="text",
        metadata={
    ...
    new_text_column: Optional[str] = field(
        default="text", metadata={"help": "Name of the new text column in data"}
    )
    dataset_config: Optional[str] = field(
        default=None, metadata={"help": "Config for translation"}
    )
    ...
    def from_name_or_path(
        ...
        if isinstance(path, str):
            try:
                ds = load_dataset(path=path, name=dataset_subset)
